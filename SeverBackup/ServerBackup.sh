...
NFSID="192.168.1.142:/tmp/nfsshare"
...
function CleanUp()
{
    if [ "$1" -eq 0 ]
    then
        echo "Script is done, You can check $LOGFIL ."
    fi
...
function LOG()
{
  ...
  then
      case $2 in
        1)
            echo "[$(date)]: MountError: The remote directory is not mount in this system!!!" | tee -a $LOGFILE
            exit 1;;
        2)
            echo "[$(date)]: BackupError: Couldn't backup $3 maybe permission dennis or disk is full" | tee -a $LOGFILE
            CleanUp 1
            exit 2;;
        3)
            echo "[$(date)]: TransistError: Backup data couldn't be copyed to $NFSDIR" | tee -a $LOGFILE
            /bin/umount $NFSDIR
            exit 3;;
        4)
            echo "[$(date)]: $3 couldn't be created" | tee -a $LOGFILE
            echo "[$(date)]: Backup is unsuccessfully" | tee -a $LOGFILE
            exit 4;;

        5)
            echo "[$(date)]: UMountError: Couldn't umount the NFS directory" | tee -a $LOGFILE
            exit 5;;
...
if [ ! -d $NFSDIR/$HOST ]
then
    echo "Creating own backup directory in NFS"
...
echo "----Clean old backup file in NFS----"
